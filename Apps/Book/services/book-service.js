...
var gBooks = (storageService.loadFromStorage(KEY)) ? storageService.loadFromStorage(KEY) : _createBooks()
    // search('bookName').then(res => {
    //     console.log(res)

// })

function query(filterBy) {
    gBooks = (storageService.loadFromStorage(KEY)) ? storageService.loadFromStorage(KEY) : _createBooks()
    if (filterBy) {
        var { title, maxPrice, minPrice } = filterBy
        maxPrice = maxPrice ? maxPrice : Infinity
        minPrice = minPrice ? minPrice : -Infinity
        const filteredBooks = gBooks.filter(book => {
            const { amount } = book.listPrice;
            return book.title.toLowerCase().includes(title.toLowerCase()) && amount >= minPrice && amount <= maxPrice
        })
...
function addReview(id, txt, stars, fullName, readAt) {
    var bookIdx = gBooks.findIndex(book => book.id === id)
    getReviews(id).then(reviews => {
        gBooks[bookIdx][reviews] = reviews.push({ txt, stars, fullName, readAt });
        storageService.saveToStorage(KEY, gBooks)
    })
...
function addBook(book) {
    if (gBooks.some(oldBook => oldBook.id === book.id)) return
    gBooks.unshift(book)
    storageService.saveToStorage(KEY, gBooks)
        // Promise.resolve()
}

function deleteReview(idx, bookId) {
    var bookIdx = gBooks.findIndex(book => book.id === bookId)
    gBooks[bookIdx].reviews.splice(idx, 1)
    storageService.saveToStorage(KEY, gBooks)
    return Promise.resolve()
}
...
function searchBook(bookName) {
    ...
    return axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${bookName}`).then(res => {
        return res.data.items.map(book => {
            return {
                ...
                pageCount: book.volumeInfo.pageCount || 'No pagecount',
                categories: book.volumeInfo.categories || 'No categories',
                thumbnail: book.volumeInfo.imageLinks.thumbnail || 'No thumbnale',
                language: book.volumeInfo.language || 'No launguage',
                listPrice: {
